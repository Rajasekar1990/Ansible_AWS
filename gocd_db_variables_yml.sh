...
mssql_login_host='172.31.45.41'
mssql_login_user='target2'
mssql_db='test_db1'
mssql_port=1434
sql_srcpath='sql_stmts1/'
sql_dirpath='sql_results1'
...
variables_yml_sedoutput=$(cat variables.yml | sed -e "s/mssql_login_host: \(.*\)/mssql_login_host: $mssql_login_host/g" \
...
echo "$variables_yml_sedoutput" > variables_test.yml


ansible_cfg_sedoutput=$(cat ansible.cfg | sed -e "s/log_path=\(.*\)/log_path=$Ansible_logpath/g" \
                                              -e "s/vault_password_file=\(.*\)/vault_password_file=$Vault_password_file/g")

echo "$ansible_cfg_sedoutput" > ansible_test.cfg